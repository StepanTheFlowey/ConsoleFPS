...
constexpr float_t fFOV = 3.14159f / 4.0f;	// Field of View
...
int main() {
  ...
  while(true) {
    ...
    chrono::duration<float_t> elapsedTime = tp2 - tp1;
    tp1 = tp2;
    ...
    for(uint16_t x = 0; x < nScreenWidth; x++) {
      ...
      constexpr float_t fStepSize = 0.05f;      // Increment size for ray casting, decrease to increase
      float_t fDistanceToWall = 0.0f;           //                                           resolution
